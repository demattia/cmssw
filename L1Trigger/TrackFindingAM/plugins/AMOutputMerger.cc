...
void AMOutputMerger::produce(edm::Event &iEvent, const edm::EventSetup &iSetup)
{
  ...
  std::auto_ptr<edmNew::DetSetVector < TTStub<Ref_PixelDigi_> > >
  TTStubsForOutput(new edmNew::DetSetVector <TTStub<Ref_PixelDigi_>>);
  ...
  std::map<edm::Ref < edmNew::DetSetVector < TTStub<Ref_PixelDigi_> >, TTStub<Ref_PixelDigi_> >, unsigned
  int > stubMap;
  ...
  for (inputIter = TTStubHandle->begin(); inputIter != TTStubHandle->end(); ++inputIter) {
    for (stubIter = inputIter->begin(); stubIter != inputIter->end(); ++stubIter) {
      ...
      edm::Ref <edmNew::DetSetVector<TTStub<Ref_PixelDigi_> >, TTStub<Ref_PixelDigi_>> tempStubRef = makeRefTo(
          TTStubHandle, stubIter);
  ...
  for (unsigned j = 0; j < TTPatternsInputTags.size(); ++j) {
    ...
    if (TTPatterns->size() > 0) {
      for (iterTTTrack = TTPatterns->begin();
        ...
        tempTTPatt.setRoadRef(iterTTTrack->roadRef());
        tempTTPatt.setPreEstimatePt((iterTTTrack->preEstimatePt()));
        tempTTPatt.setLayers(iterTTTrack->layers());
        tempTTPatt.setPrincipals(iterTTTrack->principals());
  ...
  edm::OrphanHandle <std::vector<TTTrack<Ref_PixelDigi_> >> TTPatternAcceptedHandle = iEvent.put(TTTracksForOutput,
                                                                                                 TTPatternOutputTag);
  ...
  for (inputIter = TTStubHandle->begin(); inputIter != TTStubHandle->end(); ++inputIter) {
    ...
    if (tempOutput->size() > 0) {
      typename edmNew::DetSetVector<TTStub<Ref_PixelDigi_> >::FastFiller tempOutputFiller(*TTStubsForOutput,
                                                                                          thisStackedDetId);
